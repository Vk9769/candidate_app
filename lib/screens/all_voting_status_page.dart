...
class _AllVotingStatusPageState extends State<AllVotingStatusPage> {
  ...
  List<Map<String, dynamic>> _filterData(String query) {
    ...
    for (var state in votingData) {
      ...
      for (var district in (state['districts'] as List)) {
        ...
        if (districtMatch || filteredCities.isNotEmpty) {
          filteredDistricts.add({
            ...district,
            'cities': filteredCities.isNotEmpty
                ? filteredCities
                : district['cities'],
          });
        }
      ...
      if (stateMatch || filteredDistricts.isNotEmpty) {
        filtered.add({
          ...state,
          'districts': filteredDistricts.isNotEmpty
              ? filteredDistricts
              : state['districts'],
        });
      }
  ...
  List<Map<String, dynamic>> _getCandidateVotingData() {
    List<Map<String, dynamic>> filtered = [];

    for (Map<String, dynamic> stateData
        in votingData.cast<Map<String, dynamic>>()) {
      if (stateData['state'] != widget.state) continue;

      List districts = stateData['districts'];

      List filteredDistricts = [];

      for (Map<String, dynamic> district
          in districts.cast<Map<String, dynamic>>()) {
        if (district['district'] != widget.district) continue;

        List cities = district['cities'];

        List filteredCities = [];

        for (Map<String, dynamic> city in cities.cast<Map<String, dynamic>>()) {
          if (city['city'] != widget.city) continue;

          List areas = city['areas'];

          List<Map<String, dynamic>> filteredAreas = areas
              .cast<Map<String, dynamic>>()
              .where((a) => a['area'] == widget.area)
              .toList();

          if (filteredAreas.isNotEmpty) {
            filteredCities.add({...city, 'areas': filteredAreas});
          }
        }

        if (filteredCities.isNotEmpty) {
          filteredDistricts.add({...district, 'cities': filteredCities});
        }
      }

      if (filteredDistricts.isNotEmpty) {
        filtered.add({...stateData, 'districts': filteredDistricts});
      }
    }
  ...
  Widget build(BuildContext context) {
    final filteredData = _getCandidateVotingData();
  ...
  Widget _buildSummaryCards(Map<String, int> totals) {
    ...
    final castedPercent = totalVoters > 0
        ? (casted / totalVoters * 100).toStringAsFixed(1)
        : '0';
            ...
            if (areas.isEmpty)
              ...
              ...areas.map((area) {
                ...
                final percentage = totalVotes > 0
                    ? ((casted / totalVotes) * 100).toStringAsFixed(0)
                    : '0';
